...
export default function useSubCategory() {
	...
	const [colorGuideList, setColorGuideList] = useState([]);
	...
	const getList = async ({ page, sortBy, limit, searchBy, status }: List) => {
		setLoading(true);
		try {
			const res: any = await colorGuideApi.getColorGuideList({ page, sortBy, limit, searchBy, status });
			if (res?.code == 200) {
				// Handle potential errors in the response format
				setTotalPages(res?.data?.totalPages);
				if (Array.isArray(res?.data?.docs)) {
					setColorGuideList(res?.data?.docs);
				} else {
					console.error('Unexpected response format from colorGuideApi.getCategoryList:', res.data);
				}
		...
		try {
			...
			Swal.fire({
				...
				if (result.isConfirmed) {
					...
					const res: any = await colorGuideApi.updateColorGuide(item, item?.id); 
					if (res?.code == 200) {
	...
	return { loading, getList, colorGuideList, totalPages, updateStatus }
}
