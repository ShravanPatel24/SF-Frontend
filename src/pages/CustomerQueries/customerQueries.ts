import { ContactUsService } from '@/common'
...
export default function useCustomerQueries() {
	...
	const getList = async ({ page, sortBy, limit, searchBy, status }: List) => {
		setLoading(true);
		try {
			const res: any = await ContactUsService.getFAQList({ page, sortBy, limit, searchBy, status });
			if (res?.code == 200) {
				// Handle potential errors in the response format
				setTotalPages(res?.data?.totalPages);
				if (Array.isArray(res?.data?.docs)) {
					setQueriesList(res?.data?.docs);
				} else {
					console.error('Unexpected response format from ContactUsService.getCategoryList:', res.data);
				}
		...
		try {
			...
			Swal.fire({
				...
				if (result.isConfirmed) {
					...
					const res: any = await ContactUsService.updateFAQ(item, item?.id); 
					if (res?.code == 200) {
	...
	return { loading, getList, queriesList, totalPages, updateStatus }
}
