...
const CustomFromAddClient = () => {
    ...
    useEffect(() => {
        const fetchCategory = async () => {
            if (id) {
                try {
                    const response = await ourClientApi.getOurClientDetails(id);
    ...
    const handleSubmit = async (event: any) => {
        ...
        if (form.checkValidity() === false) {
            ...
            try {
                ...
                const res: any = id ? await ourClientApi.updateOurClient(formData, clientId) : await ourClientApi.addOurClient(formData);
                    ...
                    <Row>
                        ...
                        <Col lg={previewImage ? 5 : 6}>
                            <Form.Group className="mb-3">
                                ...
                                />
                                <Form.Control.Feedback type="invalid">
                                    Please select the Client image.
                                </Form.Control.Feedback>
                            </Form.Group>
                        </Col>
                        {previewImage && (
...
const AddClient = () => {
    ...
    return (
        <>
            <PageBreadcrumb title={id ? 'Edit Client' : 'Add Client'} subName="Client" />
            <Row>
                <Col lg={12}>
                    <CustomFromAddClient />
                </Col>
