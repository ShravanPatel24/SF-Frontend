...
export default function useSubCategory() {
	...
	const getList = async ({ page, sortBy, limit, searchBy, status }: List) => {
		setLoading(true);
		try {
			const res: any = await staticContentApi.getStaticContentList({ page, sortBy, limit, searchBy, status });
			if (res?.code == 200) {
				// Handle potential errors in the response format
				setTotalPages(res?.data?.totalPages);
				if (Array.isArray(res?.data)) {
					setOurClientList(res?.data);
				} else {
					console.error('Unexpected response format from staticContentApi.getCategoryList:', res.data);
				}
		...
		try {
			...
			Swal.fire({
				...
				if (result.isConfirmed) {
					...
					const res: any = await staticContentApi.updateStaticContent(item, item?.id); 
					if (res?.code == 200) {
