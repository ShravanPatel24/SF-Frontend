...
const FAQTables = () => {
	const { loading, faqList, getList, totalPages, updateStatus } = useFaq();
	const [searchTerm, setSearchTerm] = useState('');
	const [pageNumber, setPageNumber] = useState(1);
	const [status, setStatus] = useState('');
	// const baseUrl = process.env.VITE_API_URL;
	const columns = ['S.No.', 'Question', 'Type', 'Created At', 'Status', 'Action'];
	...
	return (
		<>
			<PageBreadcrumb title="FAQ Tables" subName="Tables" />
			<CustomTable
				loading={loading}
				data={faqList}
				addButtonName="Add FAQ"
				navigationUrl="/faq"
				isImage='hide'
				columns={columns}
