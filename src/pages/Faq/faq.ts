...
export default function useFaq() {
	...
	const getList = async ({ page, sortBy, limit, searchBy, status }: List) => {
		setLoading(true);
		try {
			const res: any = await FAQApi.getFAQList({ page, sortBy, limit, searchBy, status });
			if (res?.code == 200) {
				...
				if (Array.isArray(res?.data?.docs)) {
					...
					console.error('Unexpected response format from FAQApi.getCategoryList:', res.data);
				}
		...
		try {
			...
			Swal.fire({
				...
				if (result.isConfirmed) {
					...
					const res: any = await FAQApi.updateFAQ(item, item?.id); 
					if (res?.code == 200) {
	...
	return { loading, getList, faqList, totalPages, updateStatus }
}
